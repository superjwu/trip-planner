...
export interface RawTripInput {
  ...
  anchorSlug?: string;
  /**
   * Phase J: destinations the user has already visited. Hard-excluded by
   * preFilter so the recommender never suggests them. Source is the
   * `tp-visited` cookie managed on /settings.
   */
  visitedSlugs?: string[];
}
...
export interface NormalizedTripInput {
  ...
  anchorSlug?: string;
  /** Phase J: hard-excluded from preFilter. Each slug validated against DESTINATIONS in normalize(). */
  visitedSlugs?: string[];
}
...
/**
 * Phase B presentation type: one parsed stop on a route, hydrated with
 * the destination it refers to. `parseRec` (in `app/trips/[id]/page.tsx`)
 * produces these; UI components like DestinationCard / ExpandedDestination
 * consume them. Today every rec has a single-stop array; B.4 will start
 * producing real multi-stop arrays once migration 0007 is applied.
 */
...
export interface CostBreakdown {
  ...
  /**
   * Phase B: optional, present only for multi-stop routes. Sums the road
   * transit cost between consecutive stops (haversine × $0.30/mi).
   */
  interStopDriveUsd?: number;
  /**
   * Phase B: optional per-stop breakdown. When present, the UI shows one
   * row per stop with its lodging+food+activities subtotal. The top-level
   * `lodgingUsd / foodUsd / activitiesUsd` totals are the SUM of these.
   */
  perStopCosts?: PerStopCost[];
}
...
// Bumped to 6 for Phase B (multi-stop combos): the prompt + candidate
// block shape change to support 1–3 ordered stops, and every persisted
// recommendation gains a `stops` array. Invalidates rec_cache.
...
// Bumped to v8-multi-stop for Phase B: the rec output now includes a
// `stops` array (1–3 ordered stops per pick) and the strict tool schema
// changed shape. Cache must invalidate.
export const REC_PROMPT_VERSION = "rec-v8-multi-stop";
